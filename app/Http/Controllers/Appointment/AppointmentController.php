        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($appointments)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    if ($row->app_status == AppointmentStatus::COMPLETED) {
                        $buttons[] = "<button type='button' class='waves-effect waves-light btn btn-circle btn-info btn-pdf-generate btn-xs me-1' title='Download' data-bs-toggle='modal' data-app-id='{$row->id}' data-parent-id='{$parent_id}' data-patient-id='{$row->patient->patient_id}'  data-bs-target='#modal-download'><i class='fa fa-download'></i></button>";
                    }
