...
// Calculate amount
$(document).on("input", ".price-input, .quantity-input, .packageunit-input, .packageno-input", function () {
    const row = $(this).closest("tr");
    const packageCount = parseFloat(row.find(".packageno-input").val()) || 0;
    const packageUnit = parseFloat(row.find(".packageunit-input").val()) || 0;
    const totalQuantity = packageCount * packageUnit;
    row.find(".quantity-input").val(totalQuantity);
...
$("input[type='checkbox'][name='itemmode_of_payment[]']").change(function () {
    ...
    $("#itembody tr").each(function () {
        ...
        const itemName = $(this).find("select[name='item[]']").val().trim();
        const price =
    ...
    if (allItemsFilled) {
        ...
        alert("Please add medicines and details first!");
...
$(document).ready(function () {
    ...
    $("#savePurchaseButton").on("click", function (event) {
        ...
        $("#itembody tr").each(function () {
            ...
            const itemName = $(this).find("select[name='item[]']").val().trim();
            const batchNo = $(this).find("input[name='batchNo[]']").val().trim();
            const expiryDate = $(this).find("input[name='expiryDate[]']").val();
            const packageType = $(this).find("select[name='packageType[]']").val();
            const packageCount =
                parseFloat($(this).find("input[name='packageCount[]']").val()) || 0;
            const unitsPerPackage =
                parseFloat($(this).find("input[name='unitsPerPackage[]']").val()) || 0;
            // const itemAmount =
            //     parseFloat($(this).find("input[name='itemAmount[]']").val()) || 0;
            const price =
                parseFloat($(this).find("input[name='price[]']").val()) || 0;
            const quantity =
                parseInt($(this).find("input[name='quantity[]']").val()) || 0;
            ...
            if (!itemName) {
                ...
                $(this)
                    ...
                    .text("Medicine is required.")
                    .show();
            }
            if (!batchNo) {
                isValid = false;
                $(this)
                    .find("input[name='batchNo[]']")
                    .next(".invalid-feedback")
            ...
            if (packageCount <= 0) {
                isValid = false;
                $(this)
                    .find("input[name='packageCount[]']")
                    .next(".invalid-feedback")
                    .text("Package count must be a positive number.")
                    .show();
            }
            if (unitsPerPackage <= 0) {
                isValid = false;
                $(this)
                    .find("input[name='unitsPerPackage[]']")
                    .next(".invalid-feedback")
                    .text("Units per package must be a positive number.")
                    .show();
            }
            if (price <= 0) {
                isValid = false;
    ...
    $(
        "input[type='text'],input[type='number'],input[type='date'], select, input[type='tel'], input[type='text'], textarea"
