        ...
        <div class="form-group">
            ...
            <div class="input-group form-group">
                <select class="form-control select-title" id="title" name="title" required style="width: 40%;">
                    <option value="Dr." <?php if (trim($name[0]) == 'Dr.') {
                        echo 'selected';
                    } ?>>Dr.</option>
    ...
    <div class="col-md-2">
        <div class="form-group">
            ...
            <select class="form-select" id="gender" name="gender" required>
                <option value="M" <?php if ($staffProfile->gender == 'M') {
                    echo 'selected';
                } ?>>Male</option>
                <option value="F" <?php if ($staffProfile->gender == 'F') {
                    echo 'selected';
                } ?>>Female</option>
                <option value="O" <?php if ($staffProfile->gender == 'O') {
                    echo 'selected';
                } ?>>Others</option>
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select class="form-control select2 form-select" required id="role" name="role[]"
                ...
                @foreach ($userTypes as $userType)
                    <option value="{{ $userType->id }}" <?php if ($userType->id == User::IS_ADMIN && $userDetails->is_admin) {
                        echo 'selected';
                    } ?> <?php if ($userType->id == User::IS_DOCTOR && $userDetails->is_doctor) {
                        echo 'selected';
                    } ?> <?php if ($userType->id == User::IS_NURSE && $userDetails->is_nurse) {
                        echo 'selected';
                    } ?>
                        <?php if ($userType->id == User::IS_RECEPTION && $userDetails->is_reception) {
                            echo 'selected';
    ...
    <div class="col-md-3">
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="aadhaar_no" name="aadhaar_no" placeholder="Aadhaar no"
                required value="{{ $staffProfile->aadhaar_no }}">
        </div>
    </div>
    ...
    <div class="row px-0 mx-0" id="communicationAddress">
        ...
        <div class="form-group col-md-2 ">
            <input type="text" class="form-control" id="com_address2" name="com_address2"
                placeholder="Adress line 2" required value="{{ $staffProfile->com_address2 }}">
        </div>
        ...
        <div class="form-group col-md-2 ">
            <input class="form-control" type="text" id="com_pincode" name="com_pincode" required
                placeholder="pin code" value="{{ $staffProfile->com_pincode }}">
        </div>
    </div>
</div>
