    ...
    $(document).ready(function () {
        ...
        function destroySlider() {
            ...
            if (slider.data('flexslider')) {
                slider.flexslider('destroy').removeData('flexslider');
            }
        }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (images.length === 0) {
                        ...
                        var listul = $('<ul>', { class: 'slides', id: 'imageSlides' });

                        images.forEach(function (image) {
                            var imageSrc = "{{ asset('storage/') }}/" + image.xray;
                            var filename = image.xray.split('/').pop();

                            var listItem = `<li class="fx-card-item fx-element-overlay listItem">
                                            ...
                                            <p class="flex-caption">${filename}</p>
                                        </li>`;

                            listul.append(listItem);
                        });

                        $('.flexslider').append(listul);

                        initializeSlider();
                    }

                    // Attach delete image click handler
                    $('.delete-image').click(function (e) {
                        e.preventDefault();
                        var imageToDelete = $(this).data('image');
                        deleteImage(imageToDelete);
